...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		//$rs_Section = CIBlockSection::GetList(array('left_margin' => 'desc'), array('IBLOCK_ID' =>9,'depth_level' => '1'),false,array('UF_*'));
		$ii = 0;

			$rs_element = CIBlockElement::GetList(Array("SORT"=>"asc "),array('IBLOCK_ID' =>9));
			$kk = 0;

			while($ar_element = $rs_element->GetNextElement()) {
				$arResult['element'][$kk]['row']        = $ar_element->GetFields();
				$arResult['element'][$kk]['properties'] = $ar_element->GetProperties();
				if ($arResult['element'][$kk]['properties']['FILES']["PROPERTY_TYPE"] == "F") {

					if ($arResult['element'][$kk]['properties']['FILES']["MULTIPLE"] == "Y") {
						foreach ($arResult['element'][$kk]['properties']['FILES']["VALUE"] as $kkk => $vvv) {
							$arResult['element'][$kk]['files'][$kkk] = CFile::GetFileArray($vvv);

						}
					} else {
						$arResult['element'][$kk]['files'][0] = CFile::GetFileArray($arResult['element'][$kk]['properties']['FILES']["VALUE"]);
					}
				}
				$kk++;
			}
