      ...
      <div class="text-left">
         ...
         <?
            while($ar = $arResult['rs']->Fetch()){
               ...
               if($ar['STATUS_ID'] == 0){
                  $namezz = $ar['TITLE'];
                  if($namezz != 'Показания приборов учёта' && $namezz != 'Заявка на присоединение' && $namezz != 'Оценка качества обслуживания' && $namezz != 'Заявка на временное присоединение'){

                     $posCurrMess = strpos($ar['message'], 'Сообщение:');
                     $withMess = substr($ar['message'], $posCurrMess);

                     $posTwoDucks = strpos($withMess,":")+1;

                     $ar['message'] = substr($withMess, $posTwoDucks);
