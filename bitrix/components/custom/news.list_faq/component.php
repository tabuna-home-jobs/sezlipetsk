...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		$rs_Section = CIBlockSection::GetList(array('left_margin' => 'desc'), array('IBLOCK_ID' =>30,'depth_level' => '1'),false,array('UF_*'));
		$ii = 0;
		while ( $ar_Section = $rs_Section->Fetch() )
		{
			...
			$rs_element = CIBlockElement::GetList(Array("SORT"=>"desc"),array('SECTION_ID'=>  $ar_Section ['ID']));
			$kk = 0;

			while($ar_element = $rs_element->GetNextElement()) {
				$arResult["razdel"][$ii]['element'][$kk]['row']        = $ar_element->GetFields();
				$arResult["razdel"][$ii]['element'][$kk]['properties'] = $ar_element->GetProperties();
				if ($arResult["razdel"][$ii]['element'][$kk]['properties']['FILES']["PROPERTY_TYPE"] == "F") {

					if ($arResult["razdel"][$ii]['element'][$kk]['properties']['FILES']["MULTIPLE"] == "Y") {
						foreach ($arResult["razdel"][$ii]['element'][$kk]['properties']['FILES']["VALUE"] as $kkk => $vvv) {
							$arResult["razdel"][$ii]['element'][$kk]['files'][$kkk] = CFile::GetFileArray($vvv);
