...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		$rs_Section = CIBlockSection::GetList(array('left_margin' => 'desc'), array('IBLOCK_ID' =>33,'depth_level' => '1'),false,array('UF_*'));
		$ii = 0;
		while ( $ar_Section = $rs_Section->Fetch() )
		{
			...
			$rs_Section_child = CIBlockSection::GetList(array('left_margin' => 'desc'), array('IBLOCK_ID' =>33,'SECTION_ID' =>$ar_Section['ID'],'depth_level' => '2'),false,array('UF_*'));
			$jj = 0;
			while($ar_Section_child = $rs_Section_child ->Fetch() ){
				//var_dump($ar_Section_child['DESCRIPTION']);
